...
for (dep in departments) {
  ...
  ev_rf <- vector(length=Runs) 
  ev_naive <- vector(length=Runs) 
  
  # Growing window loop
  for(b in 1:Runs)  {
    H <- holdout(d, ratio=Test, mode="incremental", iter=b, window=W, increment=S) 
    
    # Define o layout dos gráficos
    par(mfrow=c(2, 4)) # Define uma matriz de 2 linhas e 4 colunas para os gráficos
    ...
    # Plot Holt-Winters
    plot(Pred_holtwinters, type="l", col="blue", main="Holt-Winters Predictions vs Real Values", xlab="Time", ylab="Values")
    lines(d[H$ts], type="l", col="black")
    ...
    # Plot ARIMA
    plot(Pred_arima, type="l", col="green", main="ARIMA Predictions vs Real Values", xlab="Time", ylab="Values")
    lines(d[H$ts], type="l", col="black")
    ...
    # Plot mlpe
    plot(Pred_mlpe, type="l", col="red", main="mlpe Predictions vs Real Values", xlab="Time", ylab="Values")
    lines(d[H$ts], type="l", col="black")
    ...
    # Plot nnetar
    plot(Pred_nnetar, type="l", col="purple", main="nnetar Predictions vs Real Values", xlab="Time", ylab="Values")
    lines(d[H$ts], type="l", col="black")
    ...
    # Plot ETS
    plot(Pred_ets, type="l", col="orange", main="ETS Predictions vs Real Values", xlab="Time", ylab="Values")
    lines(d[H$ts], type="l", col="black")
    ...
    # Plot LM
    plot(Pred_lm, type="l", col="pink", main="LM Predictions vs Real Values", xlab="Time", ylab="Values")
    lines(d[H$ts], type="l", col="black")
   
  
    # Random Forest
    rf_results <- rf_gw(D, Test, W_rf, S, b)
    Pred_rf <- rf_results[[1]]
    ev_rf <- rf_results[[2]]
    mae_rf <- rf_results[[3]]
    rmse_rf <- rf_results[[4]]
    r2_rf <- rf_results[[5]]
    
    # Plot Random Forest
    plot(Pred_rf, type="l", col="brown", main="Random Forest Predictions vs Real Values", xlab="Time", ylab="Values")
    lines(d[H$ts], type="l", col="black")
    
    # Naive
    naive_results <- naive_gw(D, Test, W_naive, S, b)
    Pred_naive <- naive_results[[1]]
    ev_naive <- naive_results[[2]]
    mae_naive <- naive_results[[3]]
    rmse_naive <- naive_results[[4]]
    r2_naive <- naive_results[[5]]
    
    # Plot Naive
    plot(Pred_naive, type="l", col="gray", main="Naive Predictions vs Real Values", xlab="Time", ylab="Values")
    lines(d[H$ts], type="l", col="black")
    mpause()
...
for (dep in departments) {
  ...
  cat("Random Forest median NMAE:", median(results[[dep]]$ev_rf), "\n")  
  cat("Naive median NMAE:", median(results[[dep]]$ev_naive),  "\n\n") 
}
...
for (dep in departments) {
  ...
  cat("Random Forest median MAE:", median(results[[dep]]$mae_rf), "\n")  
  cat("Naive median MAE:", median(results[[dep]]$mae_naive),  "\n\n") 
}
...
for (dep in departments) {
  ...
  cat("Random Forest median RMSE:", median(results[[dep]]$rmse_rf), "\n")  
  cat("Naive median RMSE:", median(results[[dep]]$rmse_naive),  "\n\n") 
}
