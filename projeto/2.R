...
S=round(K/1) # step jump: set in this case to 4 months, a quarter #S
...
W_mlpe=W-max(timelags) # initial training window size for the D space (CasesSeries, rminer methods)
W_lm=W-max(timelags)
...
for(b in 1:Runs)  # cycle of the incremental window training (growing window)
{
  ...
  Pred_holtwinters=forecast(M,h=length(H$ts))$mean[1:Test] # multi-step ahead forecasts
  ev_holtwinters[b]=mmetric(y=d1[H$ts],x=Pred_holtwinters,metric="NMAE",val=YR)
  ...
  M_mlpe=fit(y~.,D[H_mlpe$tr,],model="mlpe") # create forecasting model
  Pred_mlpe=lforecast(M_mlpe,D,start=(length(H_mlpe$tr)+1),Test) # multi-step ahead forecasts
  ev_mlpe[b]=mmetric(y=d1[H$ts],x=Pred_mlpe,metric="NMAE",val=YR)
  ...
  cat("iter:", b, "TR from:", trinit, "to:", (trinit + length(H$tr) - 1), "size:", length(H$tr),
      ...
      "nmae:", ev_holtwinters[b], ",", ev_arima[b], ",", ev_nnetar[b], ",", ev_ets[b], ",", ev_mlpe[b], ",", ev_lm[b], "\n")
  
  mgraph(d1[H$ts],Pred_holtwinters,graph="REG",Grid=10,col=c("black","blue","green","purple","orange","red","pink"),leg=list(pos="topleft",leg=c("target","HW pred.","arima","nn","ets","mlpe","lm")))
